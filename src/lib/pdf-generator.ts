...
export async function generatePDF(data: DocumentData): Promise<Uint8Array> {
  ...
  doc.text(settings?.name?.split(' ')[0] || 'REPAIR', margin, 25)
  doc.setTextColor(59, 130, 246)
  doc.text(settings?.name?.split(' ').slice(1).join(' ') || 'FLOW', margin + (doc.getTextWidth(settings?.name?.split(' ')[0] || 'REPAIR') + 1), 25)
